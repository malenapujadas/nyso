...
export async function fetchLastPost() {
  ...
  if (error) {
    console.error(
      "[blog.js fetchLastPost] error al traer las ultimas publicaciones: ",
      error
    );
...
export async function submitSuggestion(suggestion) {
  ...
  if (error) {
    console.error(
      "[blog.js submitSuggestion] Error al enviar la sugerencia: ",
      error
    );
    throw new Error(error.message);
  }
...
export async function fetchSuggestions() {
  ...
  if (error) {
    console.error(
      "[blog.js fetchSuggestions] Error al traer sugerencias: ",
      error
    );
    throw new Error(error.message);
  }
...
export async function respondSuggestion(suggestionId, responseText) {
  ...
  if (suggestion.responded) {
    console.warn(
      "[blog.js respondSuggestion] Sugerencia ya respondida:",
      suggestionId
    );
    return suggestion;
  }
  ...
  const createdPost = await createBlogPostFromSuggestion(
    suggestion,
    responseText
  );
  const updatedSuggestion = await markSuggestionAsResponded(
    suggestionId,
    responseText
  );
...
export async function deleteSuggestion(suggestionId) {
  ...
  if (error) {
    console.error(
      "[blog.js deleteSuggestion] Error al eliminar sugerencia:",
      error
    );
    throw new Error(error.message);
  }
