<script>
import { getAllSubscriptions, updateSubscriptionStatus } from "../../services/subscriptions.js";
...
export default {
  ...
  data() {
    return {
      ...
      loading: true,
      error: null,
      processingId: null, // Para mostrar carga en un botón específico
    };
  },
  async mounted() {
...
<template>
  <div class="max-w-7xl mx-auto pb-20">
    <div class="mb-8">
      <h2 class="text-3xl font-bold text-[#4e0d05]">Gestión de Kits & Suscripciones</h2>
    ...
    <div v-else class="bg-[#ede8d7] rounded-3xl border border-[#4e0d05]/20 shadow-sm overflow-hidden">
      
      <div v-if="subscriptions.length === 0" class="p-8 text-center text-[#4e0d05]/60">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-[#4e0d05]/5 text-[#4e0d05] text-xs uppercase tracking-wider border-b border-[#4e0d05]/10">
          ...
          <tbody class="text-sm text-[#4e0d05]">
            ...
            >
              ...
              <td class="p-4 max-w-xs">
                <p>{{ sub.address }}</p>
                <p class="text-xs opacity-60">{{ sub.city }} (CP {{ sub.zip_code }})</p>
              </td>
              ...
              <td class="p-4">
                <span 
                  ...
                  :class="{
                    'bg-yellow-100 text-yellow-800 border-yellow-200': sub.status === 'pending',
                    'bg-[#3c490b]/10 text-[#3c490b] border-[#3c490b]/20': sub.status === 'active',
                    'bg-red-100 text-red-800 border-red-200': sub.status === 'cancelled'
                  }"
                >
                  {{ sub.status === 'pending' ? 'PENDIENTE' : (sub.status === 'active' ? 'ACTIVO' : 'CANCELADO') }}
                </span>
              </td>
